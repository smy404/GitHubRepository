 ...
 * Description:类型转换类
 *
 * hexStringToBytes : 16进制字符串 → byte数组
 * bytesToHexString : byte数组 → 16进制字符串
 * charToByte       : char → byte数组
 * intToByte        : int → byte数组
 * byteToInt        : byte数组 → int
 * saveDecimals     : 保留几位小数
 * nullOfString     : null → String
 * stringToByte     : String → Byte
 * stringToBoolean  : String → Boolean
 * stringToInt      : String → Int
 * stringToShort    : String → Short
